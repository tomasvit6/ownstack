...
3. Pick the runners. Use `arena runners` from `~/.claude/ownstack-models.md` when present. Otherwise default to one each on `opus` at `max` effort, `opus` at `high` effort, `sonnet` at `high` effort, and `haiku` at `medium` effort. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.
