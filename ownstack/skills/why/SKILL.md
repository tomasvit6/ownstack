...
Before spawning investigators, list the available MCP servers from the session tool list. Use the available-tools map when present. Otherwise check `.mcp.json` and the configured MCP servers for this project.
...
- `model`: your configured why-investigators model (default `sonnet` at `high` effort)
...
- `subagent_type`: `general-purpose`
- `model`: your configured why-synthesizer model (default `fable` at `max` effort)
