...
- **Worker / verifier.** Always `isolation: "worktree"` for writers unless the task needs shared local state: browser or CLI runtime verification (`claude-in-chrome`, `run`); reading local transcripts under `agent-transcripts/`; simulators and local IDE state; auth that exists only here. Worktree agents see an isolated checkout, so their briefs inline what they need or point at repo paths. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model or effort level from its worker.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the background task status via `/tasks`. Transcript mtime is not liveness.
