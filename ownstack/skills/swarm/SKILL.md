...
4. Pick the worker model from `swarm workers` in `~/.claude/ownstack-models.md` when present. Otherwise use `sonnet` at `high` effort. For a model race, name each arm's model up front.
