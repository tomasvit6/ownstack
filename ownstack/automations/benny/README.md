...
benny gives you two scheduled agents for slack issue reports. one triages each report. the other reproduces confirmed bugs and may prepare a small draft fix.
...
3. let setup enable ownstack in the target repository's `.claude/settings.json` for shared dependencies:
