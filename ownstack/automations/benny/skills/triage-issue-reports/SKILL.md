...
- Apply ownstack's `principle-separate-before-serializing-shared-state` to source coordinates.
- Apply ownstack's `principle-minimize-reader-load` and `unslop` skills to the final verdict.
...
Do a bounded source and history pass before choosing an owner or destination. Use ownstack's `how` skill to trace the path from the reported action to the observed result. Use `why` when the report looks like a regression or touches defensive code.
